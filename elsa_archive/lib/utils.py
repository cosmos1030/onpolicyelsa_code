...
def agg_loss(loss_mat, loss_mask, loss_agg_mode="token-mean", dp_size=1):
    """
    앞서 정의한 agg_loss (verl_F 의존성 제거 버전)
    """
    if loss_agg_mode == "token-mean":
        num_tokens = loss_mask.sum().clamp(min=1.0)
        return torch.sum(loss_mat * loss_mask) / num_tokens * dp_size
    return torch.sum(loss_mat * loss_mask) / loss_mat.size(0)