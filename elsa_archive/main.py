...
def main(argv):
    ...
    if FLAGS.save_model and FLAGS.prune_method != 'global_admm' and local_rank == 0:
        save_path = f'./pruned_{FLAGS.model.split("/")[-1]}_{FLAGS.prune_method}_sparsity{FLAGS.sparsity_ratio}'
        os.makedirs(save_path, exist_ok=True)
        model.save_pretrained(save_path)
        tokenizer.save_pretrained(save_path)
        logging.info(f"Pruned model saved at {save_path}")
