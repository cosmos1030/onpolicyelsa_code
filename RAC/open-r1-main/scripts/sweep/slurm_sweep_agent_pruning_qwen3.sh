...
# Usage: sbatch scripts/slurm_sweep_agent_pruning_qwen3.sh <SWEEP_ID>
# Submit N times to run N configs in parallel:
#   for i in {1..4}; do sbatch scripts/slurm_sweep_agent_pruning_qwen3.sh <SWEEP_ID>; done

SWEEP_ID=$1
if [ -z "$SWEEP_ID" ]; then
    echo "ERROR: SWEEP_ID not provided"
    echo "Usage: sbatch scripts/slurm_sweep_agent_pruning_qwen3.sh <SWEEP_ID>"
    exit 1
fi
