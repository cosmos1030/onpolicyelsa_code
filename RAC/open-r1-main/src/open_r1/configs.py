...
# ──────────────────────────────────────────────────────────────────────────────
# Step 1 -- declare dummy bases so the module can finish importing
# ──────────────────────────────────────────────────────────────────────────────
if TYPE_CHECKING:
    # Static type checkers will see the *real* classes
    from open_r1_trl.trl import ScriptArguments as _TrlScriptArguments
...
# ──────────────────────────────────────────────────────────────────────────────
# Script / trainer configs – derive (for now) from the dummy bases
# ──────────────────────────────────────────────────────────────────────────────
@dataclass
class ScriptArguments(_TrlScriptArguments):        # type: ignore[misc]
    ...
    dataset_train_split: str = field(
        default="train",
        metadata={"help": "Split name to use for training."},
    )

    dataset_config: Optional[str] = field(
        default=None,
        metadata={"help": "Optional dataset configuration name."},
    )

    dataset_config_name: Optional[str] = field(
        default=None,
        metadata={"help": "Dataset builder config (e.g. 'en' for C4)."},
    )
    dataset_split: str = field(
        default="train",
        metadata={"help": "Which split to load (train, validation, etc.)."},
    )
...
@dataclass
class GRPOConfig(_TrlGRPOConfig):          # placeholder base
    """
    ...
    vllm_mode: str = field(
        ...
        metadata={
            ...
            "choices": ["gpu_exact", "gpu_spread", "cpu"],
        },
    )
    vllm_tensor_parallel_size: int = field(
        default=1,
        metadata={"help": "Tensor-parallel degree for vLLM."},
    )
    steps_per_generation: int = field(
        default=1,
        metadata={"help": "Optimizer steps to run between generations."},
    )

    math_eval_samples: Optional[int] = field(
        default=None,
        metadata={"help": "MATH-500 samples for in-process eval (None = all 500)."},
    )
    math_eval_max_new_tokens: int = field(
        default=4096,
...
@dataclass
class SFTConfig(_TrlSFTConfig):                    # type: ignore[misc]
    """
        args for callbacks, benchmarks etc
        """
...
@dataclass
class GKDConfig(_TrlGKDConfig):                    # type: ignore[misc]
    sparse_optimizer: Optional[str] = field(default=None, metadata={"help": "The sparse optimizer to use."})
    chat_template: Optional[str] = field(default=None, metadata={"help": "The chat template to use."})
    ...
    math_eval_samples: Optional[int] = field(default=None, metadata={"help": "MATH-500 samples for in-process eval (None = all 500)."})
    math_eval_max_new_tokens: int = field(default=4096, metadata={"help": "max_new_tokens for MATH-500 eval generation."})
    math_eval_batch_size: int = field(default=8, metadata={"help": "Batch size for MATH-500 eval generation."})
...
def _patch_bases() -> None:
    """Re-create our classes so they inherit from the *real* TRL bases
    while keeping every field defined in the placeholders."""
    trl_pkg = import_module("open_r1_trl.trl")

    mapping = {
        "ScriptArguments": trl_pkg.ScriptArguments,
    ...
    g = globals()
    for name, real_base in mapping.items():
        placeholder = g[name]

        # New class: inherit from (real_base, placeholder) -- MRO keeps
        # placeholder’s custom methods / overrides, but real_base brings
        # all the standard TrainingArguments fields.
        NewCls = dataclasses.dataclass(
            type(name, (real_base, placeholder), {})
        )
        NewCls.__module__ = placeholder.__module__   # nicer repr()

        g[name] = NewCls   # overwrite in module globals

# Call immediately so importers see the rebuilt versions
_patch_bases()
