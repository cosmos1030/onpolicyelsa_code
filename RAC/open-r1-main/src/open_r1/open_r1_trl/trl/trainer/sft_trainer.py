...
try:
    ...
    if ProjectedMuon not in ZERO_SUPPORTED_OPTIMIZERS:
        ZERO_SUPPORTED_OPTIMIZERS.append(ProjectedMuon)
except Exception:
    pass
from .sft_config import SFTConfig


class SparseMaskCallback(TrainerCallback):
    """Apply sparse masks to model parameters after each optimizer step.

    DeepSpeed ZeRO-2 copies fp32 master weights → bf16 model params *after*
    the optimizer step, overwriting any in-step masking. This callback re-applies
    the mask once the full step (including that copy) is done.
    """
    def __init__(self, named_masks: dict):
        # {param_name: mask_tensor}
        self.named_masks = named_masks
    ...
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        for name, param in model.named_parameters():
            if name in self.named_masks:
                param.data.mul_(self.named_masks[name].to(param.device).to(param.dtype))
...
class SFTTrainer(Trainer):
        ...
        if args.prune:
            ...
            if args.pruning_method == "SparseGPT":
                ...
                calib_loader = make_calib_loader(
                    ...
                    tokenizer      = processing_class or AutoTokenizer.from_pretrained(model_id),
                    tokens         = args.prune_calib_tokens,
                    batch_size     = args.per_device_train_batch_size,
                    on_policy      = args.prune_on_policy, 
                    model          = model,            
                    max_new_tokens = args.prune_gen_tokens,  
                )
            ...
            save_dir = Path("/shared/public/data/rlucas") / (
                f"{Path(model_id).stem}_pruned_{int(args.prune_sparsity*100)}"
                f"_tokens{args.prune_calib_tokens}_{dataset_tag}_pruneN{args.prune_n}_pruneM{args.prune_M}"
            )
    ...
    # ---------------------------------------------------------------------
    # Hard-wired on-disk cache for every split
    # ---------------------------------------------------------------------
    def _prepare_dataset(
        ...
        from datasets import load_from_disk, Dataset as HDDataset

        SAVE_ROOT = os.environ.get(
            "OPEN_R1_SFT_CACHE_DIR",
            os.path.join(os.path.expanduser("~"), ".cache/open-r1/sft_datasets"),
        )
        cache_dir = os.path.join(SAVE_ROOT, dataset_name)

        # Constant-length wrapper needs no processing
        if isinstance(dataset, ConstantLengthDataset):
            return dataset

        # CACHE HIT  ——————————————————————————————————————————————
        if os.path.isdir(cache_dir):
            print(f"[SFTTrainer] cache hit → {cache_dir}", flush=True)
            return load_from_disk(cache_dir)

        # Otherwise preprocess and *create* the cache
        raw_ds = dataset
        column_names = list(next(iter(raw_ds)).keys())
        is_processed = "input_ids" in column_names

        map_kwargs: dict[str, Any] = {}
        if isinstance(raw_ds, HDDataset):
            map_kwargs["num_proc"] = args.dataset_num_proc

        # Only rank-0 performs the heavy work and writes the cache.
        # Other ranks wait in the barrier and then reload from disk.
        state = PartialState()
        with state.main_process_first():

            # Text-level formatting
            if formatting_func and not is_processed:
                if isinstance(raw_ds, HDDataset):
                    map_kwargs["desc"] = f"Formatting → {dataset_name}"
                raw_ds = raw_ds.map(lambda e: {"text": formatting_func(e)},
                                    batched=False, **map_kwargs)

            # ChatML / EOS / tokenisation
            if not is_processed:
                # ChatML conversion
                if isinstance(raw_ds, HDDataset):
                    map_kwargs["desc"] = f"ChatML convert → {dataset_name}"
                raw_ds = raw_ds.map(
                    maybe_convert_to_chatml,
                ...
                if is_conversational(first_row):
                    if isinstance(raw_ds, HDDataset):
                        map_kwargs["desc"] = f"Apply template → {dataset_name}"
                    raw_ds = raw_ds.map(
                        apply_chat_template,
                        fn_kwargs={"tokenizer": processing_class},
                        remove_columns="messages" if "messages" in first_row else None,
                        **map_kwargs,
                    )
                    add_special = False
                else:
                    ...
                    def _add_eos(ex):
                        if "text" in ex and not ex["text"].endswith(eos):
                            ex["text"] += eos
                        elif "completion" in ex and not ex["completion"].endswith(eos):
                            ex["completion"] += eos
                        return ex

                    if isinstance(raw_ds, HDDataset):
                        map_kwargs["desc"] = f"Add EOS → {dataset_name}"
                    raw_ds = raw_ds.map(_add_eos, **map_kwargs)
                    add_special = True

                # Tokenisation
                if isinstance(raw_ds, HDDataset):
                    map_kwargs["desc"] = f"Tokenise → {dataset_name}"

                def _tok(ex):
                    if "prompt" in ex:
                        p_ids = processing_class(ex["prompt"], add_special_tokens=add_special)["input_ids"]
                        f_ids = processing_class(ex["prompt"] + ex["completion"],
                                                 add_special_tokens=add_special)["input_ids"]
                        mask  = [0]*len(p_ids) + [1]*(len(f_ids)-len(p_ids))
                        return {"input_ids": f_ids, "completion_mask": mask}
                    return processing_class(ex[args.dataset_text_field],
                                            add_special_tokens=add_special)

                raw_ds = raw_ds.map(_tok, **map_kwargs)

            # Packing / truncation
            if packing:
                if args.max_length is None:
                    raise ValueError("packing=True requires max_length")
                if isinstance(raw_ds, HDDataset):
                    map_kwargs["desc"] = f"Pack → {dataset_name}"
                raw_ds = raw_ds.select_columns("input_ids")
                raw_ds = pack_dataset(raw_ds, args.max_length, args.packing_strategy, map_kwargs)
            elif args.max_length is not None:
                if isinstance(raw_ds, HDDataset):
                    map_kwargs["desc"] = f"Truncate → {dataset_name}"
                raw_ds = truncate_dataset(raw_ds, args.max_length, map_kwargs)
            ...
            if state.is_main_process:
                ...
                # IterableDataset → materialise to HDDataset for save_to_disk
                if isinstance(raw_ds, IterableDataset):
                    raw_ds = HDDataset.from_generator(lambda: raw_ds)

                raw_ds.save_to_disk(cache_dir)
                print(f"[SFTTrainer] cached split saved → {cache_dir}", flush=True)

        # Non-main ranks load the freshly-written cache
        if not state.is_main_process:
            raw_ds = load_from_disk(cache_dir)
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        metrics[f"{mode}_sparsity"] = sparsity 
            ...
            if sparse_opt == "ProjectedMuon":
                ...
                for name, p in opt_model.named_parameters():
                    if p.requires_grad and p.data.dim() >= 2:
                        ...
                        if not mask.all():
                            named_masks[name] = mask
                self.add_callback(SparseMaskCallback(named_masks))
                return self.optimizer
