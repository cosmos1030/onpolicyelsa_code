...
class SFTConfig(TrainingArguments):
    ...
    prune_n: Optional[int] = field(
        default=None,
        metadata={
            "help": "N in N:M sparsity ‒ number of non-zero weights to keep in each group "
                    "(set both `prune_n` and `prune_M` to enable N:M pruning)."
        },
    )
    prune_M: Optional[int] = field(
        default=None,
        metadata={
            "help": "M in N:M sparsity ‒ size of each weight group (must satisfy 0 < N ≤ M)."
        },
    )
