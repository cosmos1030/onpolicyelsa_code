...
class GKDTrainer(SFTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Top-k filtering: restrict KD loss to teacher's top-k tokens → (B, L, k)
        if self.kd_topk > 0:
            topk_idx = shifted_teacher_logits.topk(self.kd_topk, dim=-1).indices
            shifted_student_logits = shifted_student_logits.gather(-1, topk_idx)
            shifted_teacher_logits = shifted_teacher_logits.gather(-1, topk_idx)
            shifted_labels = None  # label masking irrelevant after top-k gather
    ...
    def training_step(
        ...
        """
        if self.seq_kd:
            # Temporarily disable gradient checkpointing so KV cache works during generation
            _gc_enabled = getattr(self.teacher_model, "is_gradient_checkpointing", False)
            if _gc_enabled:
