...
class GRPOConfig(TrainingArguments):
    ...
    prune: bool = field(
        default=False,
        metadata={"help": "To prune or not?"},
    )

    pruning_method: str = field(
        default="SparseGPT",
        metadata={"help": "Which pruning method e.g. SparseGPT, MP"},
    )

    prune_sparsity: float = field(
        default=0.5,
        metadata={"help": "Target sparsity ratio for SparseGPT (0.5 = 50 %)."},
    )
    prune_calib_tokens: int = field(
        default=131072,
        metadata={"help": "Number of prompt tokens for SparseGPT calibration."},
    )

    prune_N: Optional[int] = field(
        default=None,
        metadata={"help": "N in N:M structured pruning (number of non‑zeros per group)."},
    )
    prune_M: Optional[int] = field(
        default=None,
        metadata={"help": "M in N:M structured pruning (total elements per group)."},
    )
    ...
    quantize_method: str = field(
        default="W8A8",
        metadata={"help": "Quant scheme (W8A8, W4A16, …) recognised by llm-compressor"},
    )
    quantize_calib_tokens: int = field(
        default=131072,
        metadata={"help": "≈ #tokens to draw from train set for calibration"},
    )
    quantize_calib_samples: int = field(
        default=512,
        metadata={"help": "#rows to feed to the quantiser (oneshot)"},
    )
    smoothquant_strength: float = field(
        default=0.8,
        metadata={"help": "λ for SmoothQuant (0→no SQ, 1→aggressive SQ)"},
    )

    
    max_step_seconds: int | None = field(
        default=36_000,          # 10 hrs
        metadata={
            "help": (
                "Maximum wall-clock time (in seconds) a single Trainer step is "
                "allowed to run when trace_only=True.  If the time limit is "
                "hit the step is skipped and training continues."
            )
        },
    )

    score_completions: bool = field(
        default=True,
        metadata={
            "help": (
                "Whether to run the reward-function pass that scores every generated completion. "
                "Setting this to False skips reward / advantage computation and thus any PPO-style "
                "policy update that relies on those values."
            )
        },
    )
    ...
    trace_only: bool = field(default=False, metadata={"help": "Skip optimisation; just generate traces"})
    trace_tokens: Optional[int] = None 

    trace_num_shards: Optional[int] = field(
    default=None,
    metadata={
        "help": "Total number of machines running trace‑only sampling "
                "concurrently.  If set, you must also pass "
                "`trace_shard_id`."
    })
    
    trace_shard_id: Optional[int] = field(
        default=None,
        metadata={
            "help": "Index (0‑based) of *this* machine in the trace pool.  "
                    "Must be < trace_num_shards."
        },
    )
    trace_coord_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to the token‑counter JSON used by all shards.  "
                    "If omitted it is created automatically next to "
                    "`output_dir`, e.g. "
    ...
    split_base_model: bool = True   
    max_gpu_mem: str = "34GiB"   
