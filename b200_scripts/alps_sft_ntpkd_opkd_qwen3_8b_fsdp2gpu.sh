#!/bin/bash
# 2-GPU FSDP fork of alps_sft_ntpkd_opkd_qwen3_8b.sh.
#
# WHY: the single-GPU recipe takes 5.25 h for 2,048 steps on a B200 (measured
# 6.5 step/min on 2026-09-24). Running it on both cards halves that, which is
# what makes two 8B trainings fit in one session instead of one.
#
# EQUIVALENT BY CONSTRUCTION to the single-GPU recipe, the same property the
# PGD fsdp2gpu script establishes:
#     1 GPU: batch 1 x grad_accum 8 x ws 1 = 8 sequences/step
#     2 GPU: batch 1 x grad_accum 4 x ws 2 = 8 sequences/step
# Same global batch, same 134M-token budget. The 4B ALPS+SFT baselines in the
# tables are single-GPU with grad_accum=8, so this keeps both sides matched.
#
# MASTER_PORT is arg 9 so two of these can share a node without colliding.
...
# The FSDP path builds vLLM (when OPD is on) as a separate sidecar process,
# so expandable_segments is safe here -- unlike the single-GPU in-process path.
...
$TORCHRUN --nproc_per_node=2 --master_port=${MASTER_PORT} main.py \
    ...
    --seed=42 \
    --run_name_suffix="alpssft8b_${SPARSITY_TAG}_lr${LR}${TAG_SUFFIX:-}_$(basename "$DATA_PATH" .jsonl)_b200fsdp2"
