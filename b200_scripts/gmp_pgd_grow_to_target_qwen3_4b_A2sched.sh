...
JOB_TAG="gmp_pgd_grow_4b_A2sched_b200_s${SPARSITY_PCT}_lr${LR}_klb${KL_BUDGET}_pgdi${PGD_INTERVAL}_er${GROW_END_RATIO}"
...
echo "=== A2 SCHEDULE-MATCHED CONTROL (grow_rule=schedule) Qwen3-4B s${SPARSITY_PCT} kl_budget=${KL_BUDGET} lr=${LR} pgd_interval=${PGD_INTERVAL} lr_scheduler=${LR_SCHEDULER} steps=${STEPS} saliency=${SALIENCY} -- 1xB200 single-GPU, vLLM in-process (OT80/FW20) ==="
...
$PYTHON main.py \
    ...
    --gmp_pgd_grow_rule=schedule \
    --gmp_pgd_grow_rule_end_ratio=${GROW_END_RATIO} \
    ...
    --seed=42 \
    --run_name_suffix="A2sched_er${GROW_END_RATIO}_pgd_grow2target_klbudget${KL_BUDGET}_lr${LR}_pgdi${PGD_INTERVAL}_${PRUNING_SCOPE}scope_b200"
