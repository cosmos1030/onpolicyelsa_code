...
WARMUP=${20:-256}  # --lr_warmup_steps. Baseline is 256; 0 needs gmp_warmup_ratio=0 too (see below).

WARMUP_RATIO=$(python3 -c "print(0.0 if int('${WARMUP}')==0 else 0.05)")  # ratio is ONLY consulted when lr_warmup_steps==0
...
JOB_TAG="gmp_pgd_grow_4b_b200_warm${WARMUP}_s${SPARSITY_PCT}_lr${LR}_klb${KL_BUDGET}_pgdi${PGD_INTERVAL}"
...
$PYTHON main.py \
    ...
    --seed=42 \
    --run_name_suffix="pgd_grow2target_klbudget${KL_BUDGET}_lr${LR}_pgdi${PGD_INTERVAL}_${PRUNING_SCOPE}scope_b200_warm${WARMUP}"
