...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer('gmp_post_target_steps', 0, "TR-GMP only: stop training this many steps after tr_reached first flips True (dynamic -- based on when the trust-region growth actually hits final_sparsity, not a precomputed step), instead of continuing for the full remaining `steps` budget with the mask frozen. 0 = disabled (old behavior: train all the way to `steps`).")
