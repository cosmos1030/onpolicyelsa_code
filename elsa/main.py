...
if __name__ == '__main__':
    ...
    flags.DEFINE_bool('gmp_pgd_grow_to_target', False, 'PGD-driven growth: instead of a separate TR-GMP growth mechanism, PGD\'s own gmp_pgd_kl_budget self-KL bisection drives sparsity from 0 to final_sparsity directly -- _pgd_desired targets final_sparsity (not current keep-count), and revive is no longer forced equal to prune (revive saturates at min(k, revive_cand) while prune keeps going up to k), so whenever prune candidates outnumber revive candidates the accepted swap set is net-pruning, at whatever pace the self-KL budget allows. Requires --gmp_pgd=true --gmp_pgd_kl_budget=<budget> --gmp_tr_enabled=false. unstructured only for now (not yet supported for sparsity_type=N:M).')
