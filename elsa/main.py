...
def main(argv):
    global FLAGS
    if getattr(FLAGS, 'gmp_pgd_kl_budget', None) is None:
        # Not explicitly set on the command line -- default to matching
        # TR-GMP's own per-growth-event budget so PGD's per-step gate and
        # TR-GMP's periodic growth gate use the same tolerance by default.
        FLAGS.gmp_pgd_kl_budget = FLAGS.gmp_tr_kl_threshold
    ...
    # gmp_batch_size>1 + gradient checkpointing hits a flash-attn 2.8.3 backward
    # NaN on B200/Blackwell (see get_llm's attn_implementation docstring) -- force
    # sdpa in that case; flash_attention_2 (the default) is fine and faster at
    # batch_size=1, which is the common case, so leave it alone otherwise.
    _attn_impl_override = 'sdpa' if getattr(FLAGS, 'gmp_batch_size', 1) > 1 else None
    model = get_llm(FLAGS.model, FLAGS.seqlen, attn_implementation=_attn_impl_override)
    model.eval()
...
if __name__ == '__main__':
    ...
    flags.DEFINE_bool('gmp_pgd_post_target_only', False, 'Only let PGD reproject once TR-GMP growth has reached final_sparsity (or immediately, for a fixed-mask/no-growth run) -- isolates whether PGD churn during the active growth ramp helps or hurts, vs. only ever running PGD as post-target maintenance. False (default) = PGD fires from step 1 onward as before, including during growth.')
    flags.DEFINE_string('gmp_milestone_steps', '', "Comma-separated OPTIMIZER STEPS at which to save an extra HF model directory mid-run, e.g. '512,1024,1536'. --gmp_milestone_sparsities triggers on sparsity crossings, which never fire in a fixed-mask run (sparsity is constant), so this is the step-keyed equivalent: it is what lets a sparse-training run be evaluated as a TRAJECTORY (512/1024/1536/2048) instead of only at its endpoint. Each directory is a normal save_pretrained output, so it is scored with the identical lighteval path via b200_scripts/resume_eval_lighteval.sh.")
    flags.DEFINE_float('gmp_ste_shrink', 0.0, "Fisher-clean shrink of dead weights under --gmp_ste: after each optimizer.step(), w <- w*(1 - rho_t) on masked-out coordinates only. Unlike --gmp_ste_decay (AST's gradient-side SR-STE) this never enters Adam's exp_avg_sq, so it does not contaminate the Fisher that our saliency F*w^2 is built from -- measured, the gradient-side decay RAISED churn monotonically in its own strength (2.32M -> 2.93M at 1e-3 -> 4.30M at 3e-3 coordinates re-decided per PGD event). rho is a DIRECT per-step rate, deliberately not scaled by lr (an lr-scaled rho shrinks essentially nothing over 2048 steps). 0 = off.")
    flags.DEFINE_integer('gmp_ste_shrink_warmup', -1, "Steps over which --gmp_ste_shrink ramps linearly from 0 to rho_max (rho_t = rho_max*min(1, t/T_warm), AST Eq. 2's annealing shape). -1 = the first 25% of training.")
    flags.DEFINE_float('gmp_ste_decay', 0.0, "SR-STE decay strength on PRUNED weights, the half of AST (thu-ml/Adaptive-Sparse-Trainer, AAAI 2025) that --gmp_ste alone does not implement: their AdamW adds `grad += decay * w * (1 - mask)` so masked coordinates are pulled toward zero instead of drifting on a straight-through gradient no forward pass ever validates. Measured on our own 4B 2:4 STE run, without it dead coordinates drift 2-3x further from init than live ones, their saliency overtakes survivors, and the 2:4 projection re-decides ~1.9M coordinates per event and never converges (the no-STE control's last event flips 0). AST uses 6e-5 (default) / 1e-4 (train.sh). 0 = off, which reproduces plain STE. Ignored unless --gmp_ste=true.")
    flags.DEFINE_integer('gmp_ste_decay_warmup', -1, "Steps over which --gmp_ste_decay ramps linearly from 0 to its full value (AST Eq. 2 / main.py get_decay: lambda(t) = alpha*t/T0 for t<T0, alpha after). Weak early so connectivity can still be explored, strong later so the mask freezes before the weights are exported. -1 = AST's own proportion, the first 25% of training (they ramp over increase_step=10000 of max_iters=40000).")
    ...
    flags.DEFINE_float('gmp_pgd_kl_budget', None, 'Alternative to --gmp_pgd_max_swap_frac: gate PGD prunings per step by measured self-KL instead of a fixed count. Bisects the number of (lowest-importance) prune candidates accepted this step so that self-KL(pre-prune || post-prune), measured on a small cached calibration batch, stays within this budget. Revive count is always set equal to the accepted prune count (existing invariant), so revival volume is bounded for free by the same search -- revival itself is never separately KL-checked since a masked weight is architecturally zero until it grows via later gradient steps, so its instantaneous swap has no measurable effect. Defaults to --gmp_tr_kl_threshold\'s value when left unset (PGD\'s per-step gate then matches TR-GMP\'s per-growth-event budget); pass 0 explicitly to disable PGD KL-gating instead (falls back to --gmp_pgd_max_swap_frac or uncapped). N:M (sparsity_type=2:4/4:8): bisects over the group-capped eligible-prune set instead of a raw unstructured candidate set (see _pgd_nm_pre_target in gmp_trainer.py), so accepted swaps never exceed a group\'s remaining prune budget; only applies before TR-GMP growth reaches final_sparsity -- the post-target N:M maintenance step (_pgd_nm_post_target) is unaffected, since it must recompute each group exactly, not swap a partial budget.')
    ...
    flags.DEFINE_integer('gmp_pgd_interval', 1, 'Only run PGD\'s per-step reprojection (including the gmp_pgd_kl_budget self-KL bisection, the dominant per-step cost) every Nth step, decoupled from gmp_mask_interval\'s growth cadence. Default 1 = every recovery step (prior behavior). E.g. mask_interval=32 + pgd_interval=8 lets growth fire on its usual 32-step cadence while PGD reprojects only every 8th step instead of all 31 recovery steps in between.')
    flags.DEFINE_enum('gmp_pgd_topk_impl', 'bisect', ['bisect', 'kthvalue'],
                       'DIAGNOSTIC A/B flag (analysis phase, not a real default choice): which '
                       'implementation PGD\'s unstructured gmp_pgd_kl_budget branch uses to turn a '
                       'requested swap count k into an actual boolean mask. \'bisect\' (default) is the '
                       'existing 64-iteration value-threshold search. \'kthvalue\' uses torch.kthvalue on '
    ...
    flags.DEFINE_bool('gmp_pgd_nm_compensate', False, "N:M only. Before a projection zeroes the two "
                      "doomed weights of a 4-group, move their contribution onto the two survivors: "
                      "w_S += C_SS^{-1} C_SD w_D, with C the group's 4x4 input-activation covariance "
                      "(captured once per projection event from the same calibration batch the KL screen "
                      "uses). This is the OBS/SparseGPT local reconstruction restricted to the coordinates "
                      "the N:M constraint couples -- it does not change the target support, the shared-alpha "
                      "path, or the trust-region rule, it only makes the transition that gets applied "
                      "functionally cheaper. Motivated by the measurement that within a 4-group the 3rd "
                      "most important coordinate carries ~75% the importance of the 2nd, so 2:4 must always "
                      "evict coordinates the loss still cares about.")
    flags.DEFINE_float('gmp_pgd_nm_comp_ridge', 1e-6, 'Ridge added to C_SS (relative to its mean diagonal) '
                       'before the 2x2 solve in gmp_pgd_nm_compensate, for groups whose surviving inputs '
