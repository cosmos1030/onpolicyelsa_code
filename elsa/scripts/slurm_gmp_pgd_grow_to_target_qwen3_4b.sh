#!/bin/bash
# 4B port of slurm_gmp_pgd_grow_to_target_qwen3_1.7b.sh. The 4B SCOUT runs in
# the paper were trained in the B200 container (b200_scripts/), which has no
# SLURM launcher, so there was no way to run a 4B ablation on this cluster.
#
# Its first use is the "SCOUT w/o OPD" ablation: pass LOSS_WEIGHTS with a third
# field of 0. That turns off the OPD loss AND, because no vLLM engine is then
# built, leaves the PGD trust-region screening to fall back to prompt_iter --
# i.e. the fixed CoT data (see _refresh_pgd_kl_cal_batch in gmp_trainer.py).
# So the ablation is fully off-policy in both the loss and the mask screening,
# with no code change.
#
# Caveat worth stating in the paper: this trains on a different machine than
# the SCOUT checkpoints it is compared against.
#SBATCH --job-name=gmp_pgd_grow_4b
...
#
# Validated on a short (100-step, seqlen=1024, no eval) 4B s70 smoke test
...
#
# Usage: sbatch slurm_gmp_pgd_grow_to_target_qwen3_4b.sh <SPARSITY> <KL_BUDGET> [OPD_GEN_LEN] [MASK_INTERVAL] [LR_SCHEDULER] [STEPS] [LR] [DATA_PATH] [SEQLEN] [GRAD_CKPT] [WANDB_PROJECT] [SALIENCY] [PRUNING_SCOPE] [LOSS_WEIGHTS] [ROLLOUT_INTERVAL] [KD_NSAMPLES] [CALIB_SIZE] [PGD_INTERVAL] [VLLM_GPU_MEM] [JUMP_TO_TARGET] [SIDECAR]
#
...
# bisection (measured on the gradual path: 4B S70 kl_budget=0.01 reached
# target at step 464; 4B S70 kl_budget=0.02 at 272, kl_budget=99999 at 48).
# e.g.: sbatch slurm_gmp_pgd_grow_to_target_qwen3_4b.sh 0.7 0.02 512 32 cosine 2048 1e-4 \
...
echo "=== PGD-driven growth (no TR-GMP) Qwen3-4B s${SPARSITY_PCT} kl_budget=${KL_BUDGET} lr=${LR} pgd_interval=${PGD_INTERVAL} rollout_interval=${ROLLOUT_INTERVAL} jump_to_target=${JUMP_TO_TARGET} sidecar=${SIDECAR} lr_scheduler=${LR_SCHEDULER} steps=${STEPS} saliency=${SALIENCY} (OT80/FW20) ==="
