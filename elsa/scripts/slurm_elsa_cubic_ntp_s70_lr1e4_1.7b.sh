#!/bin/bash
#SBATCH --job-name=elsa_cubic_ntp_s70_1.7b
...
#SBATCH --output=/local-data/user-data/%u/elsa_cubic_ntp_s70_1.7b_%j/slurm_%j.out
exec 2>&1

# Same base config as wandb run c2pp4ud8 (ntp_admm_s0.7_lr0.0001_lmda0.005_steps8192,
# reasoning_qwen3_1.7b), but with the fixed-cubic z-schedule (admm-pruning/Boza et al.
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_cubic_ntp_s70_1.7b_${SLURM_JOB_ID}"
