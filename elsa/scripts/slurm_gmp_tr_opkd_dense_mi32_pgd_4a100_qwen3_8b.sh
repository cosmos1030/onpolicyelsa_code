...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_opkd_pgd_4a100_8b_%j.out
exec 2>&1

# TR-GMP NTP+KD+OPKD (Dense teacher) + PGD, Qwen3-8B, 4x A100-80GB FSDP
# mask_interval=32 (recovery step), milestones: s50/s60/s70
# Usage: sbatch slurm_gmp_tr_opkd_dense_mi32_pgd_4a100_qwen3_8b.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_opkd_dense_mi32_pgd_4a100_qwen3_8b.sh <KL_THRESHOLD>"}
...
echo "=== TR-GMP NTP+KD+OPKD(Dense)+PGD Qwen3-8B 4xA100 kl=${KL_THRESHOLD} ==="
...
if [ -n "$_WBID" ]; then
    ...
    $PYTHON rundb/cli.py register \
        ...
        --sub "kl=${KL_THRESHOLD}_mi32_pgd_4a100" \
