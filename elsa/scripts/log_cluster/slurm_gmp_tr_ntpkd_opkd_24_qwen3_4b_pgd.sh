#!/bin/bash
#SBATCH --job-name=tr_ntpkd_opkd_24_4b_pgd
...
#SBATCH --output=/home/doyoonkim/projects/onpolicyelsa_code/elsa/logs/tr_ntpkd_opkd_24_4b_pgd_%j.out
exec 2>&1

# Same as slurm_gmp_tr_ntpkd_opkd_24_qwen3_4b.sh but with --gmp_pgd=true --
# log_cluster port of the other server's slurm_gmp_tr_ntpkd_opkd_24_qwen3_4b_pgd.sh.
# After each optimizer step the mask is fully re-projected from fresh
# fisher-importance scores (top-k binary search at fixed sparsity) instead
# of only ever growing monotonically -- previously-kept params can be
# pruned ("prunings") and previously-pruned params can come back
# ("revivals"). Single H200, no FSDP (same as the non-PGD 2:4 script that
# already produced jobs 41545/41546) -- PGD only changes how the mask is
# recomputed each step (cheap top-k op on existing tensors), no extra
# model/activations, so it shouldn't need any more headroom than the
# non-PGD variant already proven to fit on one H200.
#
...
#
# Usage: sbatch slurm_gmp_tr_ntpkd_opkd_24_qwen3_4b_pgd.sh <SPARSITY> <LR> <KL_THRESHOLD> \
#          [MASK_INTERVAL] [L1_LAMBDA] [DATA_PATH] [WANDB_PROJECT]
# e.g.: sbatch slurm_gmp_tr_ntpkd_opkd_24_qwen3_4b_pgd.sh 0.5 1e-4 0.05
...
python main.py \
    ...
    --run_name_suffix="24_pgd_lr${LR}_mi${MASK_INTERVAL}_kl${KL_THRESHOLD}" \
    --seed=42
