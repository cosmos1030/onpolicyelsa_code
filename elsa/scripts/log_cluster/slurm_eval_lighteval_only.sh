...
# Standalone lighteval-only re-run (math500/gpqa/lcb/ifeval/gsm8k) for a
# training job that already completed + saved its checkpoint but got stuck
# during eval_full_bench -- specifically on lcb:codegeneration's dataset
...
#
# Usage: sbatch slurm_eval_lighteval_only.sh <MODEL_DIR_NAME> <WANDB_PROJECT> <WANDB_RUN_ID>
# e.g.: sbatch slurm_eval_lighteval_only.sh gmp_s50pct_lr0.0001_20260808_024943 reasoning_qwen3_1.7b bcdyv5co

MODEL_DIR_NAME=${1:?"Usage: sbatch slurm_eval_lighteval_only.sh <MODEL_DIR_NAME> <WANDB_PROJECT> <WANDB_RUN_ID>"}
