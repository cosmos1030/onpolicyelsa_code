...
#SBATCH --job-name=gmp_pgd_grow_4b_fsdp2
#SBATCH --partition=A100-80GB
#SBATCH --qos=hpgpu
# 2 GPUs, not 3: the third exists only for OPD's dedicated vLLM engine, and
# _use_fsdp_opkd is gated on gmp_onpolicy_kd_lambda > 0. This launcher's purpose
# is the w/o-OPD ablation, where no engine is built.
#SBATCH --gres=gpu:2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/gmp_pgd_grow_4b_fsdp2_%j.out
exec 2>&1
...
echo "=== PGD-driven growth (no TR-GMP) Qwen3-4B s${SPARSITY_PCT} kl_budget=${KL_BUDGET} lr=${LR} pgd_interval=${PGD_INTERVAL} rollout_interval=${ROLLOUT_INTERVAL} jump_to_target=${JUMP_TO_TARGET} FSDP2+dedicated-vLLM-GPU global_batch=$((1*GRAD_ACCUM*2)) steps=${STEPS} (OT80/FW20) ==="
