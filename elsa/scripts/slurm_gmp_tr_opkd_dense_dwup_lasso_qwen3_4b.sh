#!/bin/bash
#SBATCH --job-name=tr_opkd_dwup_4b
...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_opkd_dwup_lasso_4b_%j.out
#SBATCH --exclude=n91
exec 2>&1

# TR-GMP NTP+KD+OPKD (Dense) Qwen3-4B + dense warmup 1024 steps + LASSO
# milestone checkpointing at S50/S60/S70
# Usage: sbatch slurm_gmp_tr_opkd_dense_dwup_lasso_qwen3_4b.sh <KL_THRESHOLD> <L1_LAMBDA>

KL_THRESHOLD=${1:?"Usage: sbatch ... <KL_THRESHOLD> <L1_LAMBDA>"}
L1_LAMBDA=${2:?"Usage: sbatch ... <KL_THRESHOLD> <L1_LAMBDA>"}
...
echo "=== TR-GMP NTP+KD+OPKD(Dense)+DenseWarmup+LASSO Qwen3-4B kl=${KL_THRESHOLD} l1=${L1_LAMBDA} (milestones: s50/s60/s70) ==="
