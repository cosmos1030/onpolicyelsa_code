...
$PYTHON main.py \
    ...
    # false, not true: eval_zero_shot runs BEFORE eval_full_bench in main.py
    # (line 770 vs 863), and it needs EleutherAI/race. That dataset IS in the
    # local cache, but `datasets` no longer falls back to a cached copy under
    # HF_DATASETS_OFFLINE=1 -- in August it logged "using the latest cached
    # version" and continued (job 805476), now it raises ConnectionError. The
    # crash therefore killed the process before lighteval ever ran, losing the
    # endpoint avg5 of job 922811 entirely. Zero-shot is not part of avg5, so
    # the fix is to stop asking for it rather than to go back online for it.
