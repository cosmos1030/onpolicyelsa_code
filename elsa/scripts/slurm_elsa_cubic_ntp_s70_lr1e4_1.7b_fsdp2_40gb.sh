#!/bin/bash
#SBATCH --job-name=elsa_cubic_fsdp2_40gb
...
#SBATCH --output=/local-data/user-data/%u/elsa_cubic_fsdp2_40gb_%j/slurm_%j.out
exec 2>&1

# Cubic z-projection NTP-ADMM, Qwen3-1.7B, 2x A100-40GB FSDP (admm_use_fsdp=true).
# Combined 80GB VRAM covers the ~61GB peak seen on the single-GPU 80GB run
# (periodic zero-shot eval spike), while using idle 40GB capacity instead of
# waiting in the congested 80GB queue.
# Usage: sbatch slurm_elsa_cubic_ntp_s70_lr1e4_1.7b_fsdp2_40gb.sh <FINAL_LMDA>
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_cubic_fsdp2_40gb_${SLURM_JOB_ID}"
...
echo "=== Cubic-z NTP-ADMM Qwen3-1.7B s70, 2xA100-40GB FSDP, final_lmda=${FINAL_LMDA} ==="
