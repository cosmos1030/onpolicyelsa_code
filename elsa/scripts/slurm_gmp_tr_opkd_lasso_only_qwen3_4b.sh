#!/bin/bash
#SBATCH --job-name=tr_opkd_lasso_4b
...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_opkd_lasso_only_4b_%j.out
#SBATCH --exclude=n91
exec 2>&1

# TR-GMP NTP+KD+OPKD (Dense) Qwen3-4B + LASSO only (no dense warmup)
# Usage: sbatch slurm_gmp_tr_opkd_lasso_only_qwen3_4b.sh <KL_THRESHOLD> <L1_LAMBDA>
...
echo "=== TR-GMP NTP+KD+OPKD(Dense)+LASSO(no warmup) Qwen3-4B kl=${KL_THRESHOLD} l1=${L1_LAMBDA} ==="
