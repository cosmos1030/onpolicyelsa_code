#!/bin/bash
#SBATCH --job-name=tr_opkd_1.7b_24_lasso
...
# Same base recipe as slurm_gmp_tr_opkd_dense_qwen3_1.7b_24.sh, changes:
...
# Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_1.7b_24_lasso.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_1.7b_24_lasso.sh <KL_THRESHOLD>"}
L1_LAMBDA=1e-4
...
echo "=== TR-GMP NTP+KD+OPKD(Dense) Qwen3-1.7B 2:4 + lasso(plain,l1=${L1_LAMBDA}) + constant lr, kl=${KL_THRESHOLD} ==="
...
if [ -n "$_WBID" ]; then
    ...
    $PYTHON rundb/cli.py register \
        ...
        --sub "kl=${KL_THRESHOLD}_l1=${L1_LAMBDA}_openonly_constlr1e-4_lam1_tok256_8192steps" \
