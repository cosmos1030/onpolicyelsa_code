#!/bin/bash
#SBATCH --job-name=tr_opkd_pgd_4b
...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_opkd_pgd_4b_%j.out
exec 2>&1

# TR-GMP NTP+KD+OPKD (Dense teacher) + PGD projection, Qwen3-4B
# Same as slurm_gmp_tr_opkd_dense_mi128_qwen3_4b.sh but with --gmp_pgd=true
# PGD: after each optimizer step, re-project mask using fisher saliency (v_t * w^2)
#       logs pgd/revivals and pgd/prunings to wandb
# Usage: sbatch slurm_gmp_tr_opkd_dense_mi128_pgd_qwen3_4b.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_opkd_dense_mi128_pgd_qwen3_4b.sh <KL_THRESHOLD>"}
...
echo "=== TR-GMP NTP+KD+OPKD(Dense)+PGD Qwen3-4B kl=${KL_THRESHOLD} (milestones: s50/s60/s70) ==="
...
if [ -n "$_WBID" ]; then
    ...
    /home1/doyoonkim/miniconda3/envs/rac/bin/python rundb/cli.py register \
        ...
        --sub "kl=${KL_THRESHOLD}_mi128_pgd" \
