#!/bin/bash
#SBATCH --job-name=cubic_nopgd_1.7b
...
# ---------------------------------------------------------------------------
# PACE-MATCHED CUBIC WITHOUT PGD -- 1.7B. Fork of this file's PGD version.
#
# "고정 스케줄로 마스크를 다 채우고 나서 회복시킨다" 대조군. 기존 A2sched 대조군은
# PGD를 켜둔 채 성장 PACE만 스케줄로 바꿨기 때문에, 성장 중에도 목표 도달 후에도
# PGD의 KL 게이트된 prune/revive 스왑이 양쪽 arm에서 계속 돌았다. 4B 70%에서 그
# 대조군이 SCOUT과 0.03점 차이로 붙어버린 이유다(48.84 vs 48.81). 이 포크는 PGD를
# 완전히 끄고 GMP 자체의 cubic 램프로만 마스크를 채운다.
#
# 페이스는 --gmp_pruning_end_ratio로 맞춘다: gmp_sparse_train_steps=0이면 램프가
# step (steps * end_ratio)에서 sparsity_ratio에 도달한다. 1.7B SCOUT s70은
# kl_budget=0.01에서 step 464에 70%에 도달했으므로 er=0.2265625 -> step 464.
# (4B는 도달 step 224~232라 er=0.11328125를 썼다.)
#
# GROW_END_RATIO(env)로 덮어쓸 수 있다. KL_BUDGET/PGD_INTERVAL/CALIB_SIZE 인자는
# 받아들이지만 쓰이지 않는다 -- PGD가 꺼져 있다.
#
# Usage: GROW_END_RATIO=0.2265625 sbatch scripts/slurm_gmp_cubic_noPGD_qwen3_1.7b.sh 0.7 0.01
# ---------------------------------------------------------------------------
...
# 1.7B SCOUT s70 도달 step 464 / 2048 = 0.2265625
GROW_END_RATIO="${GROW_END_RATIO:-0.2265625}"
OPD_GEN_LEN=${3:-512}
MASK_INTERVAL=${4:-32}
...
$PYTHON main.py \
    ...
    --run_name_suffix="${RUN_TAG:+${RUN_TAG}_}cubic_noPGD_er${GROW_END_RATIO}_lr${LR}_pgdi${PGD_INTERVAL}_ri${ROLLOUT_INTERVAL}$([ "$JUMP_TO_TARGET" = "true" ] && echo "_jump")$([ "$SIDECAR" = "true" ] && echo "_sidecar")_${PRUNING_SCOPE}scope_$(basename "$DATA_PATH" .jsonl)" \
    --seed=42
