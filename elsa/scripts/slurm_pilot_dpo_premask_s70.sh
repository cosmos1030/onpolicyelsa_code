#!/bin/bash
#SBATCH --job-name=pilot_dpo_premask_s70
...
#SBATCH --exclude=n80
#SBATCH --output=/local-data/user-data/%u/job_%j/slurm/pilot_dpo_premask_s70_%j.out
exec 2>&1
...
echo "=== PILOT: Pruning-aware DPO (pre-mask ref), s70, steps=256 ==="
...
# Pruning-aware DPO:
#   ref  = π_{k-1} (deepcopy BEFORE mask update)
#   rejected = π̃_k (generation AFTER mask update, from damaged policy)
#   chosen   = dense model generations (pre-cached)
#
# gmp_dpo_reference_free=false → uses pre-mask ref model (default=False)
