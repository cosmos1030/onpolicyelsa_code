...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_opkd_4b_ef_%j.out
exec 2>&1

# TR-GMP NTP+KD+OPKD (Dense teacher) Qwen3-4B with empirical Fisher saliency
# Empirical Fisher: grad^2 on OPD cal_batch (vs Adam exp_avg_sq)
# Same config as slurm_gmp_tr_opkd_dense_qwen3_4b.sh + --gmp_fisher_source=opd_empirical
# Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_4b_ef.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_4b_ef.sh <KL_THRESHOLD>"}
...
echo "=== TR-GMP NTP+KD+OPKD(Dense) Qwen3-4B Empirical Fisher kl=${KL_THRESHOLD} (milestones: s50/s60/s70) ==="
...
[ $MAIN_EXIT -ne 0 ] && { echo "FATAL: main.py failed, skipping rundb/git"; exit $MAIN_EXIT; }
