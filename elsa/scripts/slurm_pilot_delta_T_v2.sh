...
#SBATCH --exclude=n80
#SBATCH --output=/local-data/user-data/%u/job_%j/slurm/delta_T_v2_%j.out
exec 2>&1
...
echo "=== Δ_T diagnostic v2: max_new_tokens=$MAX_NEW_TOKENS, n=64 pairs ==="
echo "SLURM_JOB_ID=$SLURM_JOB_ID  NODE=$(hostname)"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader

cd /home1/doyoonkim/projects/elsa

# mask_interval=64 → 64 pairs per refill → peek_n(64) = 64 samples per Δ_T measurement
# 512 steps / 64 interval = 8 Δ_T measurements
/home1/doyoonkim/miniconda3/envs/rac/bin/python main.py \
    ...
    --gmp_dpo_n_pairs=512 \
    --gmp_dpo_gen_batch=8 \
    --gmp_dpo_max_new_tokens=$MAX_NEW_TOKENS \
