#!/bin/bash
#SBATCH --job-name=tr_opkd_1.7b_24_lasso_fsdp
...
# (gmp_opkd_vllm_gpu_index default -1 = GPU index == world_size, no memory sharing
# with the FSDP training ranks).
# Same recipe as slurm_gmp_tr_opkd_dense_qwen3_1.7b_24_lasso.sh (single-GPU) + FSDP.
# Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_1.7b_24_lasso_fsdp4a100.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_opkd_dense_qwen3_1.7b_24_lasso_fsdp4a100.sh <KL_THRESHOLD>"}
L1_LAMBDA=1e-4
...
echo "=== TR-GMP NTP+KD+OPKD(Dense) Qwen3-1.7B 2:4 + lasso(plain,l1=${L1_LAMBDA}) + constant lr, kl=${KL_THRESHOLD}, FSDPx4 + dedicated vLLM GPU ==="
