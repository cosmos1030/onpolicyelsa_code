...
#SBATCH --partition=L40S,RTX6000ADA,A6000
#SBATCH --qos=normal
#SBATCH --gres=gpu:1
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
/home1/doyoonkim/miniconda3/envs/rac/bin/python scripts/systems/macko_vllm/measure_macko_vram.py \
    --dense "/home1/doyoonkim/.cache/huggingface/hub/models--Qwen--Qwen3-8B/snapshots/b968826d9c46dd6066d109eabc6255188de91218" \
    --sparse "${SPARSE_MODEL:?SPARSE_MODEL 필요}" \
    --tag "${TAG:-sparse}" \
    \
    --out "$OUTDIR/vllm_vram_8b_${TAG:-sparse}_${SLURM_JOB_ID}.json"
EXIT_CODE=$?
