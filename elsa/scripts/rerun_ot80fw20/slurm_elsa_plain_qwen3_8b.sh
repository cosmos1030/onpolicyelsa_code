...
#SBATCH --output=/local-data/user-data/%u/elsa_plain_8b_%j/slurm_%j.out
exec 2>&1

# Plain ELSA NTP-ADMM, Qwen3-8B, 4x A100-80GB FSDP, OT80/FW20 rerun
# No historical ELSA-plain 8B baseline exists (RERUN_PLAN_OT80FW20.md only has
# GMP/ALPS/SparseLLM for 8B) — lr/lmda extrapolated from the 1.7B/4B pattern
# (lr decreases with model size; lmda increases with sparsity).
# Budget: steps=4096, global batch=16 (batch_size=1 x grad_accum=4 x world_size=4), seqlen=2048
# lr_scheduler=constant_with_warmup (256-step warmup), lmda_schedule=constant, interval=32
#
# Usage: sbatch slurm_elsa_plain_qwen3_8b.sh <SPARSITY> <LR> <LMDA> [SPARSITY_TYPE]
...
# e.g.: sbatch slurm_elsa_plain_qwen3_8b.sh 0.5 5e-5 1e-3
#       sbatch slurm_elsa_plain_qwen3_8b.sh 0.5 5e-5 5e-3 2:4

SPARSITY=${1:?"Usage: sbatch slurm_elsa_plain_qwen3_8b.sh <SPARSITY> <LR> <LMDA> [SPARSITY_TYPE]"}
LR=${2:?"Usage: sbatch slurm_elsa_plain_qwen3_8b.sh <SPARSITY> <LR> <LMDA> [SPARSITY_TYPE]"}
LMDA=${3:?"Usage: sbatch slurm_elsa_plain_qwen3_8b.sh <SPARSITY> <LR> <LMDA> [SPARSITY_TYPE]"}
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_plain_8b_${SLURM_JOB_ID}"
...
DEBUG_LOG_COPY="/home1/doyoonkim/projects/elsa/logs/elsa_plain_8b_${SLURM_JOB_ID}_last.out"
...
echo "=== Plain ELSA NTP-ADMM Qwen3-8B s${SPARSITY_PCT} (${SPARSITY_TYPE}), lr=${LR}, lmda=${LMDA}, 4xA100-80GB FSDP (OT80/FW20) ==="
