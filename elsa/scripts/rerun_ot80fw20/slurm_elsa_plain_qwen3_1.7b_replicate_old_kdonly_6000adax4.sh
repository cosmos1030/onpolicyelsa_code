#!/bin/bash
#SBATCH --job-name=elsa_plain_1.7b_old_kdonly
...
#SBATCH --output=/local-data/user-data/%u/elsa_plain_1.7b_old_kdonly_%j/slurm_%j.out
exec 2>&1

# Same as slurm_elsa_plain_qwen3_1.7b_replicate_old.sh (steps=2048, global
# batch=8, 4xRTX6000ADA, lr_scheduler=linear/no-warmup, admm_lmda_schedule_mode
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_plain_1.7b_old_kdonly_${SLURM_JOB_ID}"
mkdir -p "$LOCAL_JOB_BASE/wandb"

DEBUG_LOG_COPY="/home1/doyoonkim/projects/elsa/logs/elsa_plain_1.7b_old_kdonly_${SLURM_JOB_ID}_last.out"
...
echo "=== ELSA plain+KD (replicate ef2zr9qy schedule) Qwen3-1.7B s${SPARSITY_PCT} (${SPARSITY_TYPE}), lmda=${LMDA}, kd=1.0/ntp=0.0, 4xRTX6000ADA (OT80/FW20) ==="
