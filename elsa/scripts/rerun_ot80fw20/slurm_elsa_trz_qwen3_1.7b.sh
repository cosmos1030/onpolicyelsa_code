...
#SBATCH --exclude=n3,n42,n46,n51,n54,n60,n77,n80,n91,n61,n64,n19,n31
#SBATCH --output=/local-data/user-data/%u/elsa_trz_1.7b_%j/slurm_%j.out
exec 2>&1

# Trust-region z-projection NTP-ADMM, Qwen3-1.7B, 4x RTX3090 FSDP, OT80/FW20 rerun.
# Same config as slurm_elsa_plain_qwen3_1.7b.sh (matches wandb run olrirliu:
# s50, lr=1e-4, lmda=1e-3, steps=4096, global batch=16) with the KL-gated
# trust-region z-projection added on top (lib/trainer.py:_compute_global_tr_z):
# sparsity grows by admm_tr_init_delta each admm_interval, gated by
# KL(Q_zprev||Q_zcand) <= admm_tr_kl_threshold; accepted growth doubles the
# step, rejected growth halves it (min admm_tr_delta_min). Once at target
# sparsity, admm_tr_gate_at_target still KL-gates further mask swaps instead
# of reselecting unconditionally every interval.
#
# Usage: sbatch slurm_elsa_trz_qwen3_1.7b.sh <SPARSITY> <LMDA> [KL_THRESHOLD] [SPARSITY_TYPE]
# e.g.: sbatch slurm_elsa_trz_qwen3_1.7b.sh 0.5 1e-3
#       sbatch slurm_elsa_trz_qwen3_1.7b.sh 0.5 1e-3 0.2

SPARSITY=${1:?"Usage: sbatch slurm_elsa_trz_qwen3_1.7b.sh <SPARSITY> <LMDA> [KL_THRESHOLD] [SPARSITY_TYPE]"}
LMDA=${2:?"Usage: sbatch slurm_elsa_trz_qwen3_1.7b.sh <SPARSITY> <LMDA> [KL_THRESHOLD] [SPARSITY_TYPE]"}
KL_THRESHOLD=${3:-0.1}
...
DEBUG_LOG_COPY="/home1/doyoonkim/projects/elsa/logs/elsa_trz_1.7b_${SLURM_JOB_ID}_last.out"
...
echo "=== TR-z NTP-ADMM Qwen3-1.7B s${SPARSITY_PCT} (${SPARSITY_TYPE}), lmda=${LMDA}, kl_threshold=${KL_THRESHOLD}, 4xRTX3090 FSDP (OT80/FW20) ==="
