#!/bin/bash
#SBATCH --job-name=elsa_plain_fsdp2_40gb
...
#SBATCH --output=/local-data/user-data/%u/elsa_plain_fsdp2_40gb_%j/slurm_%j.out
exec 2>&1

# Plain ELSA NTP-ADMM (no TR-z, no cubic), 2x A100-40GB FSDP — same hardware
# config as slurm_elsa_cubic_ntp_s70_lr1e4_1.7b_fsdp2_40gb.sh so the plain vs
# cubic comparison isn't confounded by FSDP vs single-GPU.
# Usage: sbatch slurm_elsa_plain_ntp_s70_lr1e4_1.7b_fsdp2_40gb.sh <FINAL_LMDA>
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_plain_fsdp2_40gb_${SLURM_JOB_ID}"
...
echo "=== Plain ELSA NTP-ADMM Qwen3-1.7B s70, 2xA100-40GB FSDP, final_lmda=${FINAL_LMDA} ==="
