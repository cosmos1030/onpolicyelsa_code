...
#SBATCH --output=/home1/doyoonkim/projects/elsa/logs/tr_gmp_4b_%j.out
exec 2>&1

# TR-GMP NTP+KD Qwen3-4B, milestone checkpointing at S50/S60/S70
# target=0.7, TR stops early if KL > threshold → milestones capture whatever is reached
# Usage: sbatch slurm_gmp_tr_ntp_kd_qwen3_4b.sh <KL_THRESHOLD>

KL_THRESHOLD=${1:?"Usage: sbatch slurm_gmp_tr_ntp_kd_qwen3_4b.sh <KL_THRESHOLD>"}
...
echo "=== TR-GMP NTP+KD Qwen3-4B kl=${KL_THRESHOLD} (milestones: s50/s60/s70) ==="
