#!/bin/bash
#SBATCH --job-name=elsa_cubic_lw_fsdp4_3090
...
#SBATCH --output=/local-data/user-data/%u/elsa_cubic_lw_fsdp4_3090_%j/slurm_%j.out
exec 2>&1

# Cubic z-projection NTP-ADMM, Qwen3-1.7B, 4x RTX3090 FSDP, LAYERWISE threshold
# (admm_z_layerwise=true) — per-parameter-tensor top-k, matching plain ELSA's
# comparison_group="layer" instead of one global threshold pooled across all
# params. This is the fix for the eval/sparse_loss staircase seen with the
# global-threshold cubic path (layer-to-layer sparsity reallocation as saliency
# scales drift, even with momentum/Fisher-weighted saliency).
# grad_accum=2 so global batch stays 1*2*4=8, matching every other comparison run.
# Usage: sbatch slurm_elsa_cubic_layerwise_ntp_s70_lr1e4_1.7b_fsdp4_3090.sh <FINAL_LMDA>
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_cubic_lw_fsdp4_3090_${SLURM_JOB_ID}"
...
echo "=== Cubic-z LAYERWISE NTP-ADMM Qwen3-1.7B s70, 4xRTX3090 FSDP, final_lmda=${FINAL_LMDA} ==="
