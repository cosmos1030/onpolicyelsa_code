#!/bin/bash
#SBATCH --job-name=elsa_trz_ntp_offpkd
...
#SBATCH --output=/local-data/user-data/%u/elsa_trz_ntp_offpkd_1.7b_%j/slurm_%j.out
exec 2>&1
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_trz_ntp_offpkd_1.7b_${SLURM_JOB_ID}"
...
# NTP + dataset-based KD (no generation):
#   NTP loss: cross-entropy on CoT answer tokens
#   KD loss: KL(student || teacher) on same CoT answer tokens (teacher = original dense model)
