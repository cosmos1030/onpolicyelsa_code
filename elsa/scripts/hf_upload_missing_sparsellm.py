"""One-off retroactive HF Hub upload for SparseLLM models that finished
before the HF link was captured (4B s70: model saved, wandb run has no
hub_model_id). Also writes hub_model_id/url into the run's wandb summary so
the dashboard/wandb views pick it up.
...
JOBS = [
    ("reasoning_qwen3_4b", "50ugpxrr", "qwen3_4b_sparsellm_s70pct", "cosmos1030/qwen3-4b-sparsellm-s70pct-ot80fw20"),
]

MODELS_BASE = "/home1/doyoonkim/projects/elsa/models"

# save_path doubles as the lighteval vLLM cache dir, so it can be full of
# eval-cache junk alongside the actual model files — only upload the real
...
for project, run_id, model_dir, hub_repo in JOBS:
    ...
    try:
        ...
        api.upload_folder(
            ...
            commit_message=f"SparseLLM pruned (OT80/FW20): {model_dir}",
        )
