#!/bin/bash
#SBATCH --job-name=elsa_trz_ntpkd
...
#SBATCH --output=/local-data/user-data/%u/elsa_trz_ntpkd_1.7b_%j/slurm_%j.out
exec 2>&1
...
LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_trz_ntpkd_1.7b_${SLURM_JOB_ID}"
...
# NTP+KD hybrid (kd_lambda=0.5, kd_ntp_lambda=0.5) + TR-z kl_threshold=0.5
# Teacher = same dense Qwen3-1.7B model (loaded internally by do_kd_admm routing)
$PYTHON main.py \
    --model="$MODEL" \
    --do_kd_admm=true \
    --data_path="$DATA_PATH" \
