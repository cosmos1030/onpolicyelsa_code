#!/bin/bash
#SBATCH --job-name=elsa_trz_sweep
...
#SBATCH --output=/local-data/user-data/%u/elsa_trz_sweep_1.7b_%j/slurm_%j.out
exec 2>&1
...
# Hyperparams passed via env vars
ADMM_LR=${ADMM_LR:-1e-4}
LASSO_LMDA=${LASSO_LMDA:-1e-2}
ADMM_LMDA=${ADMM_LMDA:-0.5}

LOCAL_JOB_BASE="/local-data/user-data/${USER}/elsa_trz_sweep_1.7b_${SLURM_JOB_ID}"
...
echo "CONFIG: lr=${ADMM_LR} lasso=${LASSO_LMDA} admm_lmda=${ADMM_LMDA}"
